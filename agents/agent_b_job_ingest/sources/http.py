"""The only way this agent talks to the internet.
...
The size cap is not a performance tuning knob. The Atom feed is parsed with
stdlib ElementTree, and reading an unbounded response into memory before parsing
is how a malformed or hostile document turns into an out-of-memory kill of an
unattended scheduled job. Streaming and aborting past ``max_bytes`` means the
parser never sees a document larger than we agreed to accept.
"""
...
class PoliteClient:
    ...
    def get_text(self, url: str, *, params: dict | None = None) -> str:
        ...
        silent empty string would look identical to a source with no postings.
        """
    ...
    def _get_once(self, url: str, *, params: dict | None = None) -> str:
        ...
        # errors="replace": one bad byte in a 5MB feed must not lose the other
        # 56,000 postings. Mojibake is visible downstream; an exception here is
        # a silently empty cycle.
